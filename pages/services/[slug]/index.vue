  ...
  <main class="overflow-hidden">
    ...
    <section class="relative py-20 bg-gradient-to-br from-primary/5 to-secondary/5">
      ...
      <div class="absolute inset-0 overflow-hidden pointer-events-none">
        <div class="absolute -top-1/2 -right-1/2 w-[1000px] h-[1000px] bg-primary/5 rounded-full blur-3xl"></div>
        <div class="absolute -bottom-1/2 -left-1/2 w-[800px] h-[800px] bg-secondary/5 rounded-full blur-3xl"></div>
      </div>
    </section>
    ...
    <section class="py-20">
      <div class="container">
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <div v-for="(benefit, index) in service?.benefits" :key="benefit.title" v-motion="card_enter"
            :custom="{ delay: index * 100 }" class="bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all">
    ...
    <section class="py-20 ">
      <div class="container">
        ...
        <div class="relative">
          <!-- Process Line -->
          <div class="absolute top-1/2 left-0 w-full h-1 bg-primary/20 -translate-y-1/2 hidden md:block"></div>

          <div class="grid md:grid-cols-4 gap-8">
            <div v-for="(step, index) in service?.process" :key="step.title" v-motion="fade_in"
              :custom="{ delay: index * 200 }" class="relative">
              <!-- Step Number -->
              <div
                class="w-12 h-12 bg-primary text-white rounded-full flex items-center justify-center mx-auto mb-6 relative z-10">
                {{ step.step }}
              </div>
    ...
    <section class="py-20">
      <div class="container">
        <div class="grid lg:grid-cols-2 gap-12 items-center">
          <div v-motion="slide_right" class="space-y-8">
            <h2 class="text-light-300">Advanced Features</h2>
            <div v-for="feature in service?.features" :key="feature.title" class="flex gap-6">
          ...
          <div v-motion="zoom_in" class="relative">
            <div
              class="absolute inset-0 bg-gradient-to-tr from-primary-100 to-secondary-100 rounded-3xl blur-3xl opacity-30">
            </div>
            <NuxtImg :src="service?.headerImage" :alt="service?.title" class="relative rounded-3xl shadow-lg" />
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="py-20 ">
      <div class="container">
        ...
        <div class="grid md:grid-cols-4 gap-8">
          <div v-for="(tech, index) in service?.technologies" :key="tech.name" v-motion="card_enter"
            :custom="{ delay: index * 100 }"
            class="bg-white p-6 rounded-2xl shadow-lg text-center group hover:shadow-xl transition-all">
            <Icon :name="tech.icon"
    ...
    <section class="py-20">
      <div class="container">
        ...
        <div class="max-w-3xl mx-auto">
          <div v-for="(faq, index) in service?.faqs" :key="index" v-motion="fade_in" :custom="{ delay: index * 100 }"
            class="mb-4">
            <Disclosure v-slot="{ open }">
              <DisclosureButton
                class="flex w-full justify-between rounded-lg bg-white px-6 py-4 text-left font-medium hover:bg-light-100 focus:outline-none focus-visible:ring focus-visible:ring-primary/50">
    ...
    <section class="py-20 ">
      <div class="container">
        ...
        <div class="grid md:grid-cols-3 gap-8">
          <NuxtLink v-for="relatedSlug in service?.relatedServices" :key="relatedSlug" :to="`/services/${relatedSlug}`"
            v-motion="card_enter" class="bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all">
            <div class="w-14 h-14 bg-primary/10 rounded-xl flex items-center justify-center mb-6">
      ...
      <div class="container">
        <div v-motion="fade_in"
          ...
          <p class="text-lg mb-8 max-w-2xl mx-auto opacity-90">
            Let's discuss how our {{ service?.title }} service can benefit your business
          </p>
...
import { card_enter, fade_in, hero_text, reveal, slide_right, zoom_in } from '~/lib/motions';
...
const service = ref<ServiceDetail | null>(getService(slug))
